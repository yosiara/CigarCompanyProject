...
class ContractPrintRegistry(models.TransientModel):
    _inherit = "l10n_cu_contract.print_registry"
    ...
    def print_report(self):
        ...
        if self.type == 'pdf':
            return self.env['report'].get_action(self, 'l10n_cu_hlg_contract.report_contract_template_dual', data=datas)
        if self.type == 'xls':
            ...
            return {
                ...
                'report_name': 'l10n_cu_hlg_contract.contract_single_xls_report_dual.xlsx',
                'datas': datas,
        ...
        for c in contract_obj_ids:
            ...
            contract['amount_cup'] = "{0:.2f}".format(c.total_cup).replace('.', ',')
            contract['amount_cuc'] = "{0:.2f}".format(c.total_cuc).replace('.', ',')
        ...
        return {
                    'type': 'ir.actions.report.xml',
                    'report_name': 'l10n_cu_hlg_contract_dual_currency.contract_single_doc_report_dual',
                    'datas': datas,
                }
