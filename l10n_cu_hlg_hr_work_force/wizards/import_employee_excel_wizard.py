...
class ImportEmployeesExcel(models.TransientModel):
    _name = "l10n_cu_hlg_hr_work_force.import_employee_excel_wizard"
    ...
    def action_import_employee(self):
        ...
        for row_idx in range(2, xl_sheet.nrows):
            ...
            salary = float(xl_sheet.cell(row_idx, 15).value)
            school_level_code = int(xl_sheet.cell(row_idx, 16).value)
            ...
            degree = self.env['l10n_cu_hlg_hr_work_force.degree'].search([('code', '=', degree_code)], limit=1)
        ...
        for row_idx in range(1, xl_sheet.nrows):
            ...
            if record_type.lower() == 'alta':
                motive = self.env.ref('l10n_cu_hlg_hr_work_force.supplement_motive_alta')
                record_type = 'hire'
            else:
    ...
    def import_degree(self, xl_sheet=None):
        for row_idx in range(1, xl_sheet.nrows):
            ...
            branch_science_id = self.env['l10n_cu_hlg_hr_work_force.branch_science'].search([('code','=',branch_code)], limit=1).id
            ...
            specialty_family_id = self.env['l10n_cu_hlg_hr_work_force.specialty_family'].search([('code','=',specialty_family_code)],limit=1).id

            degree_data = {'name': name, 'code': code, 'parent_code': parent_code, 'degree_level_id': degree_level_id,
                           'branch_science_id': branch_science_id, 'specialty_family_id': specialty_family_id}

            self.env['l10n_cu_hlg_hr_work_force.degree'].create(degree_data)

        #set the parent_id, match parent_code
        degree_reg = self.env['l10n_cu_hlg_hr_work_force.degree'].search([])

        for degree in degree_reg:
            if degree.parent_code != '':
                degree_id = self.env['l10n_cu_hlg_hr_work_force.degree'].search([('code', '=', degree.parent_code)]).id
