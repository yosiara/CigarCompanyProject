...
class ImportHireDropWizard(models.TransientModel):
    _name = "l10n_cu_hlg_hr_work_force.import_hire_drop_wizard"
    ...
    def action_import(self):
        ...
        return {
            ...
            'tag': 'reload',
            'params': {'menu_id': self.env.ref('l10n_cu_hlg_hr_work_force.menu_hire_drop_record').id},
        }
    ...
    def import_hire_drop(self, connection):
        ...
        contract_hr_type_obj = self.env['l10n_cu_hlg_hr_work_force.contract_hr_type']
        ...
        # hires
        try:
            cursor.execute("""Select top 1000 Id_Empleado, FechaAlta, Id_Contrato, Nombre, PrimerApellido, SegundoApellido 
                              FROM CPT_Empleados;""")
        except Exception:
            cursor.close()
        ...
        for row in cursor:
            ...
            if not hire.id:
                hire_data = {
                    ...
                    'motive_id': self.env.ref('l10n_cu_hlg_hr_work_force.supplement_motive_alta').id,
                    'name': employee.name,
        ...
        nd_motive = self.env.ref('l10n_cu_hlg_hr_work_force.supplement_motive_null')
        for row in cursor:
